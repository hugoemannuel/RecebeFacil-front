...
import { updateRecurringAction } from '@/app/actions/charges';
...
export function AutomacaoModal({ isOpen, onClose, recurringChargeId, initialData, onSuccess }: AutomacaoModalProps) {
  ...
  const { handleSubmit, watch, setValue, control } = useForm<FormData>({
    ...
    defaultValues: {
      ...
      next_generation_date: initialData?.nextGenerationDate ? toDateInput(initialData.nextGenerationDate) : '',
      custom_message: initialData?.custom_message ?? DEFAULT_TEMPLATE,
    },
  });

  const values = watch();

  const debtorName = initialData?.debtorName ?? 'Cliente';
  const valor = initialData?.amount ? formatMoney(initialData.amount) : 'R$ 0,00';
