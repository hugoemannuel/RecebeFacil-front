...
export default async function ConfiguracoesPage() {
  ...
  const [profileRes, subscriptionRes, creditorRes, automationRes, templatesRes, metricsRes] = await Promise.allSettled([
  ...
  const sentThisMonth = metricsRes.status === 'fulfilled' ? (metricsRes.value.data?.summary?.sentThisMonth ?? 0) : 0;
  ...
  return (
    <>
      ...
      <ConfiguracoesClient
        profile={profile}
        subscription={subscriptionData ? { ...subscriptionData, sentThisMonth } : null}
